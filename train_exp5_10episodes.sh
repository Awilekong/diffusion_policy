...
# ===============================================
# 实验5: 10条数据 + 冻结Encoder + Absolute Action
# ===============================================
# 极小数据量实验，测试absolute action的过拟合能力
# ===============================================

# 配置
SESSION_NAME="train_exp5_10episodes"
EXP_NAME="exp5_10episodes_frozen_encoder_absolute"
DEVICE="cuda:4"
...
echo -e "${BLUE}=========================================${NC}"
echo -e "${BLUE}  实验5: Absolute Action (10条数据)${NC}"
echo -e "${BLUE}=========================================${NC}"
echo ""
...
echo "Config:         train_diffusion_unet_exp5_10episodes.yaml"
echo ""
echo -e "${RED}关键配置:${NC}"
echo -e "  • max_train_episodes: 10 ${RED}(极小数据量)${NC}"
echo -e "  • freeze_encoder:     True"
echo -e "  • delta_action:       ${GREEN}False (绝对位姿)${NC}"
echo -e "  • num_epochs:         ${RED}1000${NC} (从600增加)"
echo ""
echo -e "${YELLOW}实验目的:${NC}"
echo "  测试在极小数据量(10条)下，absolute action能否过拟合训练集"
echo ""
...
# 发送训练命令
tmux send-keys -t ${SESSION_NAME} "python train.py --config-name=train_diffusion_unet_exp5_10episodes" C-m
