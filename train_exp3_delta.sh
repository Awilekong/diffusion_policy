...
# ===============================================
# 实验3: 310条数据 + 冻结Encoder + Delta Action
# ===============================================
# 对比实验1，只改变delta_action=True
# 验证delta action对收敛的影响
# ===============================================

# 配置
SESSION_NAME="train_exp3_delta"
EXP_NAME="exp3_310episodes_frozen_encoder_delta"
DEVICE="cuda:3"
...
echo -e "${BLUE}=========================================${NC}"
echo -e "${BLUE}  实验3: Delta Action (310条数据)${NC}"
echo -e "${BLUE}=========================================${NC}"
echo ""
...
echo "Config:         train_diffusion_unet_exp3_delta.yaml"
echo ""
echo -e "${RED}关键配置:${NC}"
echo -e "  • max_train_episodes: 310 (全部数据)"
echo -e "  • freeze_encoder:     True"
echo -e "  • ${RED}delta_action:        True${NC} ${YELLOW}(相比实验1的唯一差异)${NC}"
echo -e "  • num_epochs:         400"
echo ""
echo -e "${YELLOW}对比实验1:${NC}"
echo "  实验1: 310条 + 冻结encoder + 400 epochs + ${GREEN}absolute action${NC}"
echo "  实验3: 310条 + 冻结encoder + 400 epochs + ${RED}delta action${NC}"
echo ""
...
# 发送训练命令
tmux send-keys -t ${SESSION_NAME} "python train.py --config-name=train_diffusion_unet_exp3_delta" C-m
