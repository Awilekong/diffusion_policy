...
class DiffusionPolicyWrapper(BasePolicy):
    """
    包装 Diffusion Policy 为 BasePolicy 接口
    处理所有归一化、反归一化和数据转换
    内部维护历史帧队列以满足 n_obs_steps 的要求
    """
    ...
    def __init__(self, ckpt_path: str, device: str = 'cuda', debug: bool = True):
        ...
        #   - 例如 horizon=16, n_obs_steps=2
        #   - 输入 [obs_t-1, obs_t]，预测 [act_t-1, act_t, ..., act_t+14]
        #   - 但 act_t-1 是过去的，真正可用的是 [act_t, ..., act_t+14]
        #   - 可用动作数 = 16 - 1 = 15 = horizon - (n_obs_steps - 1) = horizon - n_obs_steps + 1
        ...
        print(f"\n💡 推理时图像处理流程:")
        print(f"   1. 客户端发送: 任意分辨率 (H, W, 3) uint8")
        print(f"   2. 历史队列: 统一为首张图像的分辨率")
    ...
    def _init_obs_history(self):
        """初始化历史观测队列"""
        self.obs_history = {
            'images': {},  # {camera_idx: deque of images}
            'state': None,  # deque of states
        }
        print(f"📦 初始化历史帧队列 (n_obs_steps={self.n_obs_steps})")
    ...
    def infer(self, obs: dict) -> dict:
        ...
        Args:
            obs: 观测数据字典，统一格式:
                ...
                注意：单张图像输入，内部会自动组装成 n_obs_steps 的队列
        ...
        for obs_key, camera_idx in image_mapping.items():
            if obs_key in obs:
                ...
                # 初始化该相机的队列
                if camera_idx not in self.obs_history['images']:
                    self.obs_history['images'][camera_idx] = deque(maxlen=self.n_obs_steps)
                    # 用第一帧填充整个队列
                    for _ in range(self.n_obs_steps):
                        self.obs_history['images'][camera_idx].append(img.copy())
                    if self.debug:
                        print(f"📸 初始化相机 {camera_idx} ({obs_key}) 历史队列, shape: {img.shape}")
                else:
                    # 检查形状一致性
                    expected_shape = self.obs_history['images'][camera_idx][0].shape
                    if img.shape != expected_shape:
                        raise ValueError(
                            f"相机 {camera_idx} 图像形状不一致! "
                            f"期望: {expected_shape}, 实际: {img.shape}"
                        )
                    # 添加新帧
                    self.obs_history['images'][camera_idx].append(img.copy())
        ...
        if 'observation/state' in obs:
            ...
            # 修复夹爪动作：对最后一维取反 (gripper: 0=open, 1=close)
            # state[-1] = 1.0 - state[-1]
            
            if self.obs_history['state'] is None:
                self.obs_history['state'] = deque(maxlen=self.n_obs_steps)
                # 用第一个状态填充整个队列
                for _ in range(self.n_obs_steps):
                    self.obs_history['state'].append(state.copy())
                if self.debug:
                    print(f"🤖 初始化状态历史队列, shape: {state.shape}")
            else:
                # 检查形状一致性
                expected_shape = self.obs_history['state'][0].shape
                if state.shape != expected_shape:
                    raise ValueError(
                        f"状态形状不一致! 期望: {expected_shape}, 实际: {state.shape}"
                    )
                # 添加新状态
                self.obs_history['state'].append(state.copy())
                if self.debug:
                    # 只打印形状，不打印具体数值
                    print(f"📊 状态队列更新, 队列长度: {len(self.obs_history['state'])}, shape: {state.shape}")
        
        # 转换历史队列为 diffusion policy 内部格式
        env_obs = {}
        
        # 组装图像队列: (n_obs_steps, H, W, C)
        for camera_idx, img_queue in self.obs_history['images'].items():
            img_array = np.stack(list(img_queue), axis=0)  # (n_obs_steps, H, W, C)
            env_obs[f'camera_{camera_idx}'] = img_array
        
        # 组装状态队列: (n_obs_steps, state_dim)
        if self.obs_history['state'] is not None:
            state_array = np.stack(list(self.obs_history['state']), axis=0)
            env_obs['robot_eef_pose'] = state_array
        
        # ========== 调试: 打印 env_obs 维度 ==========
        if self.debug:
            print(f"\n{'='*60}")
            print(f"🔍 Step {self.step_counter} - 推理数据流追踪")
            print(f"{'='*60}")
            print(f"\n1️⃣  env_obs (历史队列组装后):")
            print(f"   说明: 将 n_obs_steps={self.n_obs_steps} 帧历史数据堆叠")
            for key, value in env_obs.items():
                if 'camera' in key:
                    print(f"   {key}: shape={value.shape} (n_obs_steps, H, W, C), dtype={value.dtype}")
                else:
                    print(f"   {key}: shape={value.shape} (n_obs_steps, state_dim), dtype={value.dtype}")
        ...
        if self.debug:
            ...
            print(f"   说明: 图像 resize、归一化到[0,1]、转为(n_obs_steps,C,H,W)")
            for key, value in obs_dict_np.items():
                if len(value.shape) == 4:  # 图像 (T,C,H,W)
                    print(f"   {key}: shape={value.shape} (n_obs_steps, C, H, W), dtype={value.dtype}")
                else:  # 低维数据
        ...
        if self.debug:
            ...
            print(f"   说明: 添加 batch 维度 (1, n_obs_steps, ...), 在policy内部会进行归一化")
            for key, value in obs_dict_tensor.items():
                if len(value.shape) == 5:  # 图像 (B,T,C,H,W)
                    print(f"   {key}: shape={value.shape} (batch, n_obs_steps, C, H, W), dtype={value.dtype}")
                else:  # 低维数据
                    print(f"   {key}: shape={value.shape}, dtype={value.dtype}")
...
def main(input, port, host, device, debug):
    """启动 Diffusion Policy 远程推理服务器"""
    
    print("=" * 60)
    print("Diffusion Policy 远程推理服务器")
    print("=" * 60)
